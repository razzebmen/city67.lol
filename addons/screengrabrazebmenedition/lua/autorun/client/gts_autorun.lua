...
local sizes = { 11, 12, 13, 14, 15, 16, 18, 20, 25, 30, 35, 40, 50, 60, 80 }
...
-- Language selection removed; Russian only.

GTS.Interface = GTS.Interface or {}

GTS.Interface.Theme = {}
GTS.Interface.Theme.Dark = { background = Color( 10, 10, 10 ) }
...
GTS.Interface.Languages.russian = {
	...
	["quality_screen"] = "Качество скрина :",
	["recommended_quality"] = "Рекомендуемое качество : 75",
	["quality_note"] = "Примечание: чем выше качество, тем дольше ожидание",
	["select_language"] = "Выбор языка",
	["cancel"] = "Отмена",
	["close"] = "Закрыть",
	["advanced_mode"] = "Использовать расширенный режим",
...
function GTS.Interface.GetText( key )
	local selectedLang = GTS.Interface.Languages.russian
	if ( selectedLang and selectedLang[key] ) then
...
function GTS.Interface.OpenLoading()
	...
	GTS.Interface.LoadingPnl.Paint = function( self, w, h )
		...
		draw.SimpleText( GTS.Interface.GetText( "loading" ), "Arial_GTS_40_Bold", w/2, h/2, color_white, 1, 1 )
	end
end 
...
function GTS.Interface.OpenPlayerSelection()
	...
	frame.Paint = function( self, w, h )
		...
		draw.SimpleText( GTS.Interface.GetText( "menu_title" ), "Arial_GTS_35_Bold", w/2, 32.5, color_white, 1, 1 )
		surface.SetDrawColor( Color( 160, 0, 0 ) )
		for i = 0, 4 do
	...
	local btns = { frame.informationBtn, frame.closeBtn }
	local xPos = 790
	...
	frame.profilBtn:SetSize( 200, 25 )
	local function fitButtonText( btn, text )
		local fonts = {
			"Arial_GTS_20",
			"Arial_GTS_18",
			"Arial_GTS_16",
		...
		for _, font in ipairs( fonts ) do
			...
			if ( textW <= btn:GetWide() - 8 ) then
				btn:SetFont( font )
				btn:SetText( text )
				return
			end
		end
		btn:SetFont( fonts[#fonts] )
		btn:SetText( text )
	end
	fitButtonText( frame.profilBtn, GTS.Interface.GetText( "show_profile" ) )
	...
	frame.requestBtn.DoClick = function( self )
		...
		if ( frame.selectedPlayer ) then
			local delay = math.Rand( 0.25, 1.1 )
			timer.Simple( delay, function()
				if ( not IsValid( frame ) or not IsValid( frame.selectedPlayer ) ) then
					return
				end
	...
	local methodTextX = frame.methodCheck:GetX() + frame.methodCheck:GetWide() + 8
	local methodTextW = frame:GetWide() - methodTextX - 10

	frame.methodLbl = vgui.Create( "DLabel", frame )
	frame.methodLbl:SetPos( methodTextX, frame.methodCheck:GetY() - 2 )
	frame.methodLbl:SetSize( methodTextW, 10 )
	frame.methodLbl:SetFont( "Arial_GTS_15_Bold" )
	frame.methodLbl:SetText( GTS.Interface.GetText( "advanced_mode" ) )
	frame.methodLbl:SetTextColor( color_white )
	frame.methodLbl:SetWrap( true )
	frame.methodLbl:SetAutoStretchVertical( true )
	frame.methodLbl:SizeToContentsY()

	frame.methodInfoLbl = vgui.Create( "DLabel", frame )
	frame.methodInfoLbl:SetPos( methodTextX, frame.methodLbl:GetY() + frame.methodLbl:GetTall() - 2 )
	frame.methodInfoLbl:SetSize( methodTextW, 10 )
	frame.methodInfoLbl:SetFont( "Arial_GTS_13" )
	frame.methodInfoLbl:SetText( GTS.Interface.GetText( "advanced_mode_info" ) )
	frame.methodInfoLbl:SetTextColor( Color( 200, 200, 200 ) )
	frame.methodInfoLbl:SetWrap( true )
	frame.methodInfoLbl:SetAutoStretchVertical( true )
	frame.methodInfoLbl:SizeToContentsY()
...
function GTS.Interface.OpenInformation()
	if ( IsValid( GTS.Interface.PSPnl ) ) then
		...
		bg.Paint = function( self, w, h )
			...
			surface.DrawRect( 0, 0, w, h )
			draw.SimpleText( "by razebmen for CITY-67", "Arial_GTS_35_Bold", w/2, 380, color_white, 1, 0 )
		end
		
		local infoImg = vgui.Create( "DImage", bg )
		local closeBtn = vgui.Create( "DButton_GTS", bg )
		
		infoImg:SetSize( 160, 160 )
		infoImg:SetPos( pW/2-80, 190 )
		infoImg:SetImage( "gts/razebmen.jpg" )
		infoImg:SetKeepAspect( true )
...
function GTS.Interface.OpenSettings()
	if ( IsValid( GTS.Interface.PSPnl ) ) then
		...
		local russianBtn = vgui.Create( "DButton_GTS", bg )
		russianBtn:SetPos( pW/2-75, 415 )
		russianBtn:SetSize( 150, 45 )
		russianBtn:SetFont( "Arial_GTS_25_Bold" )
		russianBtn:SetText( "Русский" )
		russianBtn.zoomEnabled = false
		russianBtn.DoClick = function( self )
			if ( GetConVar( "gts_language" ):GetString() ~= "russian" ) then
				GetConVar( "gts_language" ):SetString( "russian" )
				GTS.Interface.PSPnl.profilBtn:SetText( GTS.Interface.GetText( "show_profile" ) )
				GTS.Interface.PSPnl.requestBtn:SetText( GTS.Interface.GetText( "request_screen" ) )
				if ( not IsValid( GTS.Interface.PSPnl.selectedPlayer ) ) then
					GTS.Interface.PSPnl.nickLbl:SetText( GTS.Interface.GetText( "select_player" ) )
