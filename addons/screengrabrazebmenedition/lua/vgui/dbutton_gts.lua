...
function PANEL:Init() 
	self.primaryColor = Color( 15, 15, 15 )
	self.iconBackgroundColor = Color( 20, 20, 20 )
	self.iconColor = Color( 235, 235, 235 )
	self.borderColor = Color( 200, 0, 0 )
	self.textColor = Color( 235, 235, 235 )
	
	self.borderSize = 1
	self.borderOnHovered = false
...
function PANEL:Paint( w, h )
	...
	local bPosX, bPosY = self:LocalToScreen( 0, 0 )
	local clipX1, clipY1 = bPosX, bPosY
	local clipX2, clipY2 = bPosX + w, bPosY + h
	if ( self.parentScissor ) then
		...
		clipX1 = math.max( clipX1, pPosX - zoomVal )
		clipY1 = math.max( clipY1, pPosY - zoomVal )
		clipX2 = math.min( clipX2, pPosX + pSizeW + zoomVal * 2 )
		clipY2 = math.min( clipY2, pPosY + pSizeH + zoomVal * 2 )
	end
