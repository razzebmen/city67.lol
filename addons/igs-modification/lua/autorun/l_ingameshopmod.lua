...
elseif CLIENT then
	...
	-- Конвар FCVAR_REPLICATED приходит от сервера только после InitPostEntity.
	-- Читаем его там, а не сразу в autorun — иначе получаем пустую строку.
	hook.Add("InitPostEntity", "IGS_ClientInit", function()
		hook.Remove("InitPostEntity", "IGS_ClientInit")

		local version = GetConVar("igs_version"):GetString()
		if not tonumber(version) then
			-- Конвар ещё пустой — ждём ещё немного (на случай очень медленной репликации)
			timer.Simple(1, function()
				version = GetConVar("igs_version"):GetString()
