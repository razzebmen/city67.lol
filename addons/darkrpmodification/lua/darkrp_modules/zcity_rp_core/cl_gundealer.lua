-- [ZCITY_PORT] Файл автоматически мигрирован из gamemodes/zcity/.../modes/roleplay/cl_gundealer.lua
...
-- Меню покупки оружия для Продавца Оружия в режиме Roleplay
-- Оригинальное TDM Buy Menu, адаптированное под RP:
--   - деньги берутся из GetRoleplayMoney() вместо TDM_Money
--   - нет ограничения по времени (убран таймер)
--   - открывается по F3 только для профессии "Продавец Оружия"
...
-- Глобальная переменная — переиспользуем ту же что и TDM, чтобы не было двух меню одновременно
GD_RP_BuyMenu = GD_RP_BuyMenu or nil

local function OpenGunDealerMenu()
	if IsValid(GD_RP_BuyMenu) then
		GD_RP_BuyMenu:Remove()
		GD_RP_BuyMenu = nil
		return
	end
	...
	GD_RP_BuyMenu = vgui.Create("ZFrame")
	local Frame = GD_RP_BuyMenu
	...
	for k, category in SortedPairsByMemberValue(BuyItems, "Priority") do
		...
		for n, Item in pairs(category) do
			...
			function BuyBtn:DoClick()
				net.Start("roleplay_gundealer_buy")
	...
	-- Кнопка покупки энтити "Магазин оружия"
	local ShopBtn = vgui.Create("DButton", Frame)
	ShopBtn:DockMargin(10, 0, 10, 6)
	ShopBtn:Dock(BOTTOM)
	ShopBtn:SetText("Купить Магазин оружия  —  15000$")
	ShopBtn:SetTextColor(Color(255, 220, 100))
	ShopBtn:SetFont("ZB_TDM_DESC")
	ShopBtn:SetHeight(ScrH() * 0.03)
	ShopBtn.Paint = function(self, w, h)
		local col = self:IsHovered() and Color(80, 60, 10, 220) or Color(50, 40, 5, 180)
		draw.RoundedBox(4, 0, 0, w, h, col)
		surface.SetDrawColor(200, 160, 30, 150)
...
-- F3 для оружейника отключён — меню открывается только через радиальное E-меню

-- Кнопка в радиальном меню для Продавца Оружия
hook.Add("radialOptions", "roleplay_gundealer_radial", function()
	-- [ZCITY_PORT] было: проверка ZCity_RP.name == 'roleplay'; теперь всегда true

	local ply = LocalPlayer()
	if not ply:Alive() then return end
	if ply:GetNWString("RoleplayJob", "Гражданский") ~= "Продавец Оружия" then return end
