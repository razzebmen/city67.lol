...
function zb.GetMapPoints( pointGroup, forceupdatepoints ) -- Загрузить точки в память игры... На клиенте будет примерно такая же функция.
    -- Возвращаем ПУСТУЮ ТАБЛИЦУ вместо false: все вызывающие делают #result
    -- или table.CopyFromTo, и краш на boolean ничему не помогает.
    if not zb.CreateMapDir() then PrintMessage( HUD_PRINTTALK, "sv_points.lua: map folder dosen't exist?" ) return {} end
    if not zb.Points[pointGroup] then PrintMessage( HUD_PRINTTALK, "sv_points.lua: point group " .. "\"" .. pointGroup .. "\"" .. " doesn't exist." ) return {} end
