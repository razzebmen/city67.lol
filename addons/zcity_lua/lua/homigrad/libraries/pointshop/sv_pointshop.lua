...
hook.Add( "PlayerInitialSpawn","Pointshop_OnInitSpawn", function( ply )
	...
	local query = mysql:Select("hg_pointshop")
		...
		query:Callback(function(result)
			-- Async-гонка: игрок мог отключиться → ply невалиден, а ветка else
			-- попыталась бы INSERT по уже существующему steamid (Duplicate entry).
			...
			if (istable(result) and #result > 0 and result[1].donpoints) then
				...
				local insertQuery = mysql:InsertIgnore("hg_pointshop")
