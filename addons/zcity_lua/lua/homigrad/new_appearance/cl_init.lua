...
function hg.Appearance.LoadAppearanceFile(strFile_name)
	...
	if not tblAppearance then return false, "повреждённый JSON [data/zcity/appearances/" .. strFile_name .. ".json]" end
	if not hg.Appearance.AppearanceValidater(tblAppearance) then return false, "файл повреждён [data/zcity/appearances/" .. strFile_name .. ".json]"  end
...
function DrawAccesories(ply, ent, accessories,accessData, islply, force, setup)
	...
	if not IsValid(ply.modelAccess[accessories]) then
		...
		if accessData["SubMats"] then
			for idx, mat in pairs(accessData["SubMats"]) do
				model:SetSubMaterial(idx, mat)
			end
		end
