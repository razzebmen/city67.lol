...
function PANEL:PostInit()
    ...
    -- Поле имени readonly: всегда показывается Steam ник игрока
    ...
    NameEntry:SetText(LocalPlayer():Nick())
    NameEntry:SetEditable(false)
    NameEntry:SetEnabled(false)
    NameEntry:SetTooltip("Имя берётся из Steam, изменение отключено")
    NameEntry:Dock(FILL)
    NameEntry:DockMargin(ScreenScale(4), 0, ScreenScale(4), 0)
    NameEntry:SetContentAlignment(5)
    -- Сбрасываем AName чтобы редактор не пытался сохранить старое значение
    main.AppearanceTable.AName = LocalPlayer():Nick()
    function NameEntry:Paint(w, h)
        draw.RoundedBox(4, 0, 0, w, h, Color(20, 20, 25, 240))
        surface.SetDrawColor(colors.scrollbarBorder)
        surface.DrawOutlinedRect(0, 0, w, h, 1)
        self:DrawTextEntryText(Color(170,170,170), colors.selectionBG, Color(170,170,170))
    end
