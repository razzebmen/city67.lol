...
local function ForceApplyAppearance(ply, tbl, noModelChange)
    ...
    -- Кастомные имена отключены: всегда Steam ник
    ply:SetNWString("PlayerName", (ply.IsPlayer and ply:IsPlayer()) and ply:Nick() or "")
...
-- Мин. интервал между мгновенными применениями внешности (анти-спам).
-- Должен быть >= окна tWaitResponse (3с), чтобы на один серверный запрос
-- проходило ровно одно применение. Легитимные применения инициирует сам
-- сервер на спавне (sh_default), а RP-респавн ~15с — кулдаун им не мешает.
local APPEARANCE_APPLY_COOLDOWN = 5
...
net.Receive("Get_Appearance",function(len,client)
    ...
    -- АНТИ-СПАМ: мгновенное применение внешности разрешено ТОЛЬКО как ответ на
    -- запрос сервера (tWaitResponse, ставится в ApplyAppearance на спавне).
    -- Без этой проверки клиент мог слать Get_Appearance вручную и бесконечно
    -- менять/рандомить внешность на лету (особенно с пустой таблицей → random).
    if not IsValid(client) then return end
    if not tWaitResponse[client] or tWaitResponse[client] < CurTime() then return end

    -- доп. кулдаун: один Get_Appearance-ответ на один серверный запрос
    if client.NextAppearanceApply and client.NextAppearanceApply > CurTime() then return end
    client.NextAppearanceApply = CurTime() + APPEARANCE_APPLY_COOLDOWN
...
function ApplyAppearanceRagdoll(ent, ply)
    ...
    -- Ник трупа = Steam ник владельца
    ent:SetNWString("PlayerName", IsValid(ply) and ply:Nick() or "")
