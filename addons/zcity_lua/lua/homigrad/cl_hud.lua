...
local function CreateRadialMenu(options_arg, bAutoClose)
	...
	menuPanel.Paint = function(self, w, h)
		...
		if !paining then
			-- ZCity RP: над деньгами показываем РП-профессию (NWString "RoleplayJob"),
			-- а не homigrad-роль (lply.role.name у RP-игроков пустая → строка была пустой).
			local roleText  = (lply.role and lply.role.name) or ""
			local roleColor = (lply.role and lply.role.color) or incoentCol
			if CurrentRound and CurrentRound() and CurrentRound().name == "roleplay" then
				local rpJob = lply:GetNWString("RoleplayJob", "")
				if rpJob ~= "" then
					roleText  = rpJob
					roleColor = team.GetColor(lply:Team()) or roleColor
				end
			end
			...
			draw.SimpleText(roleText,"HomigradFontGigantoNormous" ,scrW * 0.0215 * viewLerp,scrH * 0.098, colBack, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			...
			draw.SimpleText(roleText,"HomigradFontGigantoNormous" ,scrW * 0.02 * viewLerp,scrH * 0.095, roleColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			
			-- Отображение баланса в режиме Roleplay
			if CurrentRound and CurrentRound().name == "roleplay" and GetRoleplayMoney then
				local money = GetRoleplayMoney()
				draw.SimpleText(money .. "$", "HomigradFontGigantoNormous", scrW * 0.0215 * viewLerp, scrH * 0.154, colBack, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
				draw.SimpleText(money .. "$", "HomigradFontGigantoNormous", scrW * 0.02 * viewLerp, scrH * 0.15, Color(100, 220, 100), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			end
			
			-- Текст с биндом в правом нижнем углу отключен
			--[[
...
-- Админ-группы, которым доступно контекстное меню (C).
-- Спавн-меню по F1 доступно админам и модераторам (с урезанным меню для них).
-- Q всегда отдает управление ZCity радиальному меню.
local HG_CONTEXT_MENU_GROUPS = {
	dadmin      = true,
	admin       = true,
	superadmin  = true,
	dsuperadmin = true,
...
local function CanUseContextMenu(ply)
	if not IsValid(ply) then return false end
	local group = string.lower(ply:GetUserGroup() or "")
	return HG_CONTEXT_MENU_GROUPS[group] == true or ply:IsAdmin()
end

local function CanOpenSpawnMenu(ply)
	if not IsValid(ply) then return false end
	local group = string.lower(ply:GetUserGroup() or "")
	return HG_SPAWNMENU_GROUPS[group] == true or ply:IsAdmin()
end

local function IsRoleplayRound()
	return CurrentRound and CurrentRound() and CurrentRound().name == "roleplay"
end

hook.Add( "PlayerBindPress", "PlayerBindPressExample2huy", function( ply, bind, pressed )
	local lp = IsValid(ply) and ply or lply
	if not IsValid(lp) then return end

	-- F3 (gm_showspare1) в RP режиме используется для переключения курсора,
	-- открытие spawn menu здесь отключено. См. modes/roleplay/cl_roleplay.lua.

	if bind == "+menu" then
		if (lp.organism and lp.organism.otrub) then
			return true
		end
	...
	if bind == "+menu_context" then
		if CanUseContextMenu(lp) then return end
		return true
	end
end)
...
hook.Add("radialOptions", "7", function()
    ...
    if ply:Alive() and not organism.otrub and hg.GetCurrentCharacter(ply) == ply then
        -- Отключаем жесты в режиме roleplay
        if CurrentRound and CurrentRound().name == "roleplay" then return end
...
-- Подсказки при наведении на объекты отключены
--[[
local hint
