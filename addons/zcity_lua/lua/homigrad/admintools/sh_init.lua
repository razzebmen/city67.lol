...
-- Доп. ULib-группы донат-админки получают тот же доступ к C-меню,
-- что и стандартные admin/superadmin.
local DCMENU_ADMIN      = { dadmin = true, dsuperadmin = true }
local DCMENU_SUPERADMIN = { dsuperadmin = true }

function plyMeta:ZCTools_GetAccess( bSAdmin )
    local group = self:GetUserGroup()
    if bSAdmin then
        if self:IsSuperAdmin() then return true end
        if DCMENU_SUPERADMIN[group] then return true end
    else
        if self:IsAdmin() then return true end
        if DCMENU_ADMIN[group] then return true end
    end
