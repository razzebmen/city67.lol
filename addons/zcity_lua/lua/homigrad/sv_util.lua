...
hook.Add("Player Think", "homigrad-dropholstered", function(ply)
	...
	-- Игрок в зоне спавна (Roleplay) — НИЧЕГО не выкидываем. Принудительный
	-- холстер на спавне иначе моментально дропал бы только что подобранное
	-- оружие из-за флага NoHolster.
	local inSpawnZone = false
	if ply.RP_SpawnPos then
		local radius = ROLEPLAY_SPAWN_PROTECT_RADIUS or 300
		if radius > 0 and ply:GetPos():DistToSqr(ply.RP_SpawnPos) <= radius * radius then
			inSpawnZone = true
		end
	end
	...
	for i = 1, #weps do
		...
		-- Холодное оружие (на базе weapon_melee) НИКОГДА не выкидывается —
		-- его можно носить в инвентаре. Урон оно при холстере не наносит.
		if wep.Base == "weapon_melee" then continue end

		-- В зоне спавна ничего не дропаем (см. выше).
		if inSpawnZone then continue end
...
function hgWreckBuildings(blaster, pos, power, range, ignoreVisChecks) -- taken from JMod -- this so unstable shit, can crush your game
	...
	timer.Create("ProcessCheck_" .. index, 0, 0, function()
		if not IsValid(blaster) or coroutine.status(co) == "dead" then
			timer.Remove("ProcessCheck_" .. index)
			return
		end
		local ok, err = pcall(coroutine.resume, co)
		if not ok then
			ErrorNoHalt("[ProcessCheck] coroutine.resume упал: " .. tostring(err) .. "\n")
			timer.Remove("ProcessCheck_" .. index)
		end
	end)
end
...
if util.IsBinaryModuleInstalled("eightbit") then
	...
	if success then
		...
		print("[ZBattle] Модуль eightbit успешно загружен")
	else
		MsgC(Color(255, 165, 0), "[ZBattle] Не удалось загрузить модуль eightbit: " .. tostring(err) .. "\n")
		MsgC(Color(255, 165, 0), "[ZBattle] Модуль eightbit отключен, но сервер продолжит работу\n")
	end
else
