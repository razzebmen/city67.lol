...
hook.Add("HG.InputMouseApply", "fakeCameraAngles2", function(tbl)
	...
	-- lply.organism приходит с сервера не сразу — при заходе он ещё nil,
	-- а HG.InputMouseApply уже стреляет с первого движения мыши.
	local org   = lply.organism
	local larma = org and org.larmamputated
	local rarma = org and org.rarmamputated

	if (wep.weight or wep.visualweight) and ((wep.weight and wep.weight > 0 or wep.visualweight and wep.visualweight > 0) or larma or consmul > 0.3) then
		ViewPunch3(Angle(-y / 50 / 16, x / 50 / 16, 0) * math.min(((wep.visualweight ~= nil and wep.visualweight > 0) and wep.visualweight) or wep.weight, 10) / 3 / (1 - consmul * 0.5) * (larma and 4 or 1) * (rarma and 2 or 1))
	end
...
function playerMeta:GetPlayerName()
	-- Только Steam ник, кастомные имена отключены
	return self:Nick()
end
