...
if CLIENT then
	...
	net.Receive("zChatMessage", function(len)
		...
		local supressed = hook.Run("OnPlayerChat", speaker, text, false, IsValid(speaker) and speaker:Alive(), bWhisper)
		if !supressed then
	...
	hook.Add("OnPlayerChat", "ZChatDead", function(ply, text, bTeam, bDead, bWhisper)
		if ( ply:IsPlayer() and !ply:Alive() ) then
			chat.AddText( dead, "*DEAD* ", ghost, ply:GetPlayerName(), ghost, ": "..text )
			return true
		end
	end)
else
