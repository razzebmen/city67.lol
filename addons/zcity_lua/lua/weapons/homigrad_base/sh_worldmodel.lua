...
local function DrawWorldModel(self, force)
	...
	if IsValid(self.worldModel) and willdraw then
		...
		local isHolstered = not localdraw and IsValid(owner) and owner.GetActiveWeapon and owner:GetActiveWeapon() ~= self
		if self.holsteredClip and self.holsteredClip > 0 and isHolstered and self.holsterAnchorPos then
			local clipNormal = self.holsterAnchorAng:Forward() * -1
			local clipPos = self.holsterAnchorPos + self.holsterAnchorAng:Forward() * self.holsteredClip
...
function SWEP:WorldModel_Transform_Holstered()
	...
	if IsValid(ent) then
		...
		if self.holsteredAttachment and self.holsteredAttachment ~= "" then
			local attId = ent:LookupAttachment(self.holsteredAttachment)
			if not attId or attId <= 0 then return end
			local att = ent:GetAttachment(attId)
			if not att then return end
			basePos, baseAng = att.Pos, att.Ang
		else
		...
		local desiredPos, desiredAng = LocalToWorld(localPos, localAng, basePos, baseAng)
		
		desiredAng:RotateAroundAxis(ang:Right(), self.addAngle[2])
		desiredAng:RotateAroundAxis(ang:Forward(), self.addAngle[1])
		
		self.holsterAnchorPos = desiredPos
		self.holsterAnchorAng = desiredAng
...
function hg.RenderWeapons(ent, owner)
	...
	if owner.GetWeapons then
		...
		-- «Гангстер-стайл»: только ОДИН — первый по индексу — пистолет в
		-- инвентаре показывается за пояс (ValveBiped.Bip01_Pelvis), и только
		-- если он сейчас НЕ в руке. Если первый пистолет активный (в руке),
		-- второй за пояс НЕ суётся. Так пистолет за поясом всегда один и тот
		-- же конкретный экземпляр.
		--
		-- ВАЖНО: shouldntDrawHolstered у Deagle стояло true → DrawWorldModel
		-- (sh_worldmodel.lua:322) пропускал holstered, пистолет мелькал только
		-- во время self.deploy. Для primaryPistol снимаем флаг.
		local primaryPistol = nil
		for i = 1, #weps do
			local w = weps[i]
			if w.ishgweapon then
				local ht = w.HoldType
				if ht == "pistol" or ht == "revolver" then
					primaryPistol = w
					break
				end
			end
		...
		for i = 1, #weps do
			...
			if wep2.ishgweapon and wep2 ~= wep then
				local ht = wep2.HoldType
				local isPistol = (ht == "pistol" or ht == "revolver")
				if isPistol then
					-- Любой пистолет, не являющийся primaryPistol — не рисуем.
					if wep2 ~= primaryPistol then continue end
					-- primaryPistol → на пояс
					if wep2.holsteredBone ~= "ValveBiped.Bip01_Pelvis" then
						wep2.holsteredBone = "ValveBiped.Bip01_Pelvis"
						wep2.holsteredPos = Vector(-2, 2, 4.5)
						wep2.holsteredAng = Angle(20, -90, -90)
					end
					wep2.shouldntDrawHolstered = false
				end
				DrawWorldModel(wep2)
			end
		end
...
local function hg_FormatVec(vec)
	return string.format("Vector(%.2f, %.2f, %.2f)", vec.x, vec.y, vec.z)
end

local function hg_FormatAng(ang)
	return string.format("Angle(%.2f, %.2f, %.2f)", ang.p, ang.y, ang.r)
end

local function hg_GetClosestBody(ply, radius)
	...
	for _, ent in ipairs(ents.FindInSphere(origin, radius)) do
		if ent:IsRagdoll() then
			...
			if not bestDistSqr or dist < bestDistSqr then
				best = ent
				bestDistSqr = dist
			end
		end
...
-- Консольные переменные для настройки позиций в реальном времени
local hg_holster_edit_mode = CreateClientConVar("hg_holster_edit_mode", "0", false, false, "Enable holster position editing mode", 0, 1)
local hg_holster_pos_x = CreateClientConVar("hg_holster_pos_x", "0", false, false, "Holster position X offset")
local hg_holster_pos_y = CreateClientConVar("hg_holster_pos_y", "0", false, false, "Holster position Y offset")
local hg_holster_pos_z = CreateClientConVar("hg_holster_pos_z", "0", false, false, "Holster position Z offset")
local hg_holster_ang_p = CreateClientConVar("hg_holster_ang_p", "0", false, false, "Holster angle Pitch")
local hg_holster_ang_y = CreateClientConVar("hg_holster_ang_y", "0", false, false, "Holster angle Yaw")
local hg_holster_ang_r = CreateClientConVar("hg_holster_ang_r", "0", false, false, "Holster angle Roll")

-- Команда для начала редактирования позиции оружия
concommand.Add("hg_holster_edit_start", function(ply, cmd, args)
	ply = IsValid(ply) and ply or LocalPlayer()
	if not IsValid(ply) then return end
	
	local wep = ply:GetActiveWeapon()
	if not (IsValid(wep) and ishgweapon(wep)) then
		print("[Holster Edit] Нужно держать оружие в руках!")
		return
	end
	
	-- Сохраняем текущие значения в консольные переменные
	RunConsoleCommand("hg_holster_pos_x", tostring(wep.holsteredPos.x))
	RunConsoleCommand("hg_holster_pos_y", tostring(wep.holsteredPos.y))
	RunConsoleCommand("hg_holster_pos_z", tostring(wep.holsteredPos.z))
	RunConsoleCommand("hg_holster_ang_p", tostring(wep.holsteredAng.p))
	RunConsoleCommand("hg_holster_ang_y", tostring(wep.holsteredAng.y))
	RunConsoleCommand("hg_holster_ang_r", tostring(wep.holsteredAng.r))
	RunConsoleCommand("hg_holster_edit_mode", "1")
	
	print("=== РЕЖИМ РЕДАКТИРОВАНИЯ ПОЗИЦИИ ОРУЖИЯ ===")
	print("Оружие: " .. wep:GetClass())
	print("Текущая позиция: " .. hg_FormatVec(wep.holsteredPos))
	print("Текущий угол: " .. hg_FormatAng(wep.holsteredAng))
	print("")
	print("УПРАВЛЕНИЕ:")
	print("  Позиция X: hg_holster_pos_x <значение>")
	print("  Позиция Y: hg_holster_pos_y <значение>")
	print("  Позиция Z: hg_holster_pos_z <значение>")
	print("  Угол Pitch: hg_holster_ang_p <значение>")
	print("  Угол Yaw: hg_holster_ang_y <значение>")
	print("  Угол Roll: hg_holster_ang_r <значение>")
	print("")
	print("Для сохранения: hg_holster_edit_save")
	print("Для отмены: hg_holster_edit_cancel")
	print("==========================================")
end)

-- Команда для сохранения позиции
concommand.Add("hg_holster_edit_save", function(ply, cmd, args)
	ply = IsValid(ply) and ply or LocalPlayer()
	if not IsValid(ply) then return end
	
	if hg_holster_edit_mode:GetInt() == 0 then
		print("[Holster Edit] Режим редактирования не активен! Используйте hg_holster_edit_start")
		return
	end
	...
	local newPos = Vector(
		hg_holster_pos_x:GetFloat(),
		hg_holster_pos_y:GetFloat(),
		hg_holster_pos_z:GetFloat()
	)
	local newAng = Angle(
		hg_holster_ang_p:GetFloat(),
		hg_holster_ang_y:GetFloat(),
		hg_holster_ang_r:GetFloat()
	)
	
	print("=== СОХРАНЕНИЕ ПОЗИЦИИ ОРУЖИЯ ===")
	print("Оружие: " .. wep:GetClass())
	print("Кость: " .. (wep.holsteredBone or "ValveBiped.Bip01_Pelvis"))
	print("")
	print("ДОБАВЬТЕ В ФАЙЛ ОРУЖИЯ:")
	print("SWEP.holsteredBone = \"" .. (wep.holsteredBone or "ValveBiped.Bip01_Pelvis") .. "\"")
	print("SWEP.holsteredPos = " .. hg_FormatVec(newPos))
	print("SWEP.holsteredAng = " .. hg_FormatAng(newAng))
	print("SWEP.shouldntDrawHolstered = false")
	print("=================================")
	
	RunConsoleCommand("hg_holster_edit_mode", "0")
end)

-- Команда для отмены редактирования
concommand.Add("hg_holster_edit_cancel", function(ply, cmd, args)
	RunConsoleCommand("hg_holster_edit_mode", "0")
	print("[Holster Edit] Редактирование отменено")
end)

-- Хук для применения изменений в реальном времени
hook.Add("Think", "hg_holster_edit_realtime", function()
	if hg_holster_edit_mode:GetInt() == 0 then return end
	...
	for i = 1, #weps do
		local wep = weps[i]
		if wep.ishgweapon and wep ~= ply:GetActiveWeapon() then
			-- Применяем временные значения для предпросмотра
			wep.holsteredPos = Vector(
				hg_holster_pos_x:GetFloat(),
				hg_holster_pos_y:GetFloat(),
				hg_holster_pos_z:GetFloat()
			)
			wep.holsteredAng = Angle(
				hg_holster_ang_p:GetFloat(),
				hg_holster_ang_y:GetFloat(),
				hg_holster_ang_r:GetFloat()
			)
			wep.shouldntDrawHolstered = false
		end
	end
end)

concommand.Add("hg_holster_dump", function(ply, cmd, args)
	ply = IsValid(ply) and ply or LocalPlayer()
	if not IsValid(ply) then return end

	local tr = ply:GetEyeTrace()
	local wep = IsValid(tr.Entity) and tr.Entity or ply:GetActiveWeapon()
	if not (IsValid(wep) and ishgweapon(wep)) then
		print("hg_holster_dump: no hg weapon")
		return
	end

	local useAttachment = args[1] == "att"
	local baseName = useAttachment and (args[2] or "anim_attachment_RH") or (args[1] or "ValveBiped.Bip01_Pelvis")
	local nearestBody = hg_GetClosestBody(ply, 200)
	local ent = IsValid(nearestBody) and nearestBody or (IsValid(ply.FakeRagdoll) and ply.FakeRagdoll or ply)

	local basePos, baseAng
	if useAttachment then
		local attId = ent:LookupAttachment(baseName)
		if not attId or attId <= 0 then
			print("hg_holster_dump: attachment not found: " .. baseName)
			return
		end

		local att = ent:GetAttachment(attId)
		if not att then
			print("hg_holster_dump: attachment data missing")
			return
		end

		basePos, baseAng = att.Pos, att.Ang
	else
		local boneId = ent:LookupBone(baseName)
		if not boneId then
			print("hg_holster_dump: bone not found: " .. baseName)
			return
		end

		local matrix = ent:GetBoneMatrix(boneId)
		if not matrix then
			print("hg_holster_dump: bone matrix missing")
			return
		end
	...
	local modelEnt = IsValid(wep.worldModel) and wep.worldModel or wep
	local modelPos, modelAng = modelEnt:GetPos(), modelEnt:GetAngles()

	local posAfterFake, angAfterFake = modelPos, modelAng
	if wep.ShouldUseFakeModel and wep:ShouldUseFakeModel() and (wep.FakePos or wep.FakeAng) then
		local invFake = Matrix()
		invFake:SetTranslation(wep.FakePos or vector_origin)
		invFake:SetAngles(wep.FakeAng or angle_zero)
		invFake:Invert()
		posAfterFake, angAfterFake = LocalToWorld(invFake:GetTranslation(), invFake:GetAngles(), modelPos, modelAng)
	end

	local invWorld = Matrix()
	invWorld:SetTranslation(wep.WorldPos or vector_origin)
	invWorld:SetAngles(wep.WorldAng or angle_zero)
	invWorld:Invert()

	local desiredPos, desiredAng = LocalToWorld(invWorld:GetTranslation(), invWorld:GetAngles(), posAfterFake, angAfterFake)
	local localPos, localAng = WorldToLocal(desiredPos, desiredAng, basePos, baseAng)

	print("---- hg_holster_dump ----")
	print("class: " .. wep:GetClass())
	if useAttachment then
		print("holsteredAttachment = \"" .. baseName .. "\"")
	else
		print("holsteredBone = \"" .. baseName .. "\"")
	end
	print("holsteredPos = " .. hg_FormatVec(localPos))
	print("holsteredAng = " .. hg_FormatAng(localAng))
	print("-------------------------")

	if args[#args] == "apply" then
		if useAttachment then
			wep.holsteredAttachment = baseName
		else
			wep.holsteredBone = baseName
			wep.holsteredAttachment = nil
		end
		wep.holsteredPos = localPos
		wep.holsteredAng = localAng
		wep.shouldntDrawHolstered = false
		print("hg_holster_dump: applied to weapon instance")
	end
end)
