...
psay:defaultAccess( ULib.ACCESS_ADMIN ) -- city67: psay только модератор и выше (выдаётся через groups.txt)
...
local function CanSeeAsay( ply )
	if not IsValid( ply ) then return false end
	if zcreports and zcreports.config and zcreports.config.WhoCanReceiveReports then
		return zcreports.config.WhoCanReceiveReports[ ply:GetUserGroup() ] == true
	end
	return ULib.ucl.query( ply, seeasayAccess )
end

function ulx.asay( calling_ply, message )
	if IsValid( calling_ply ) and not CanSeeAsay( calling_ply ) then
		ULib.tsayError( calling_ply, "Эта команда доступна только администрации.", true )
		return
	end
...
-- "@" shortcut убран, чтобы ElysiumReports мог обрабатывать @ как подачу жалобы.
-- Админы могут использовать "ulx asay <msg>" из консоли или биндить кнопку.
local asay = ulx.command( CATEGORY_NAME, "ulx asay", ulx.asay, nil, true, true )
