...
-- Ищет самый свежий валидный groups_backup*.txt и парсит его.
-- Возвращает таблицу групп либо nil если ничего годного не найдено.
local function tryLoadFromBackup( basePath )
	local folder   = basePath:GetPathFromFilename()
	local filename = basePath:GetFileFromFilename():sub( 1, -5 ) -- без ".txt"

	-- Собираем все существующие бэкапы (groups_backup.txt, groups_backup2.txt, ...)
	local candidates = {}
	local first = folder .. filename .. "_backup.txt"
	if ULib.fileExists( first, true ) then
		table.insert( candidates, first )
	end
	local num = 2
	while true do
		local p = folder .. filename .. "_backup" .. num .. ".txt"
		if not ULib.fileExists( p, true ) then break end
		table.insert( candidates, p )
		num = num + 1
	end

	-- Перебираем от самого свежего (последнего созданного) к самому старому
	for i = #candidates, 1, -1 do
		local data = ULib.fileRead( candidates[i], true )
		if data then
			local parsed = ULib.parseKeyValues( ULib.removeCommentHeader( data, "/" ) )
			if parsed and parsed[ ULib.ACCESS_ALL ] then
				Msg( "[ULib] Восстановление groups.txt из бэкапа: " .. candidates[i] .. "\n" )
				return parsed
			end
		end
	end
...
local function reloadGroups()
	...
	if not ucl.groups or not ucl.groups[ ULib.ACCESS_ALL ] then
		needsBackup = true
		-- Сначала пытаемся восстановиться из последнего валидного бэкапа,
		-- чтобы не терять кастомные группы (moderator/dmoderator/dadmin/...)
		-- если хостинг покорёжил основной файл.
		local fromBackup = tryLoadFromBackup( path )
		if fromBackup then
			ucl.groups = fromBackup
		else
			-- Бэкапов нет или все битые — тогда уже дефолт.
			local err2
